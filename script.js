...
function minimax(board, depth, maximizingPlayer) {
    ...
    // Se houver um vencedor, retorna a pontuação correspondente
    if (winner !== null) {
        return scores[winner];
    }

    // Se for a vez do jogador maximizador ('X')
    if (maximizingPlayer) {
...
function renderBoard() {
    const buttons = document.querySelectorAll('.game button'); // Obtém todos os botões do tabuleiro
...
function checkWinnerAndDisplayMessage() {
    ...
    const winner = checkWinner(board);
    // Obtém o elemento de mensagem do HTML
    ...
    // Atualiza e exibe o placar agregado na tela com base no vencedor ou empate
    updateScore(winner);
}
